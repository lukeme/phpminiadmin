...
<br>";
 ...
 <b>selected tables</b>";
    ...
    while ($row = mysql_fetch_row($sth)) {
        $sqldr.="<tr class='" . $rc[$swp = !$swp] . "' onmouseover='tmv(this)' onmouseout='tmo(this)' onclick='tc(this)' sc=''>";
        for ($i = 0; $i < $fields_num; $i++) {
            $v = $row[$i];
            $more = '';
            if ($is_sht && $v) {
                ...
                $v = "<input type='checkbox' name='cb[]' value=\"$vq\"></td>"
                        . "<td><a href=\"$url&q=select+*+from+$vq+order+by+id+desc\">$v</a></td>"
                        . "<td>" . $row[1] . "</td>"
    ...
    <html>
        <head><title>phpMiniAdmin</title>
            ...
            <script type="text/javascript">
                ...
                function tc(tr) {
                    if (tr.sc.indexOf('s') >= 0) {
                        tr.className = tr.sc.replace(' s', '');
                    } else {
                        tr.className = tr.sc + ' s';
                    }
                    tr.sc = tr.className;
                }
                function lschk(skip) {
...
function print_login() {
    print_header();
    echo '<center><h3>Access protected by password</h3><div style="width:400px;border:1px solid #999999;background-color:#eeeeee">Password:<input type="password"name="pwd"value=""><input type="hidden"name="login"value="1"><input type="submit"value=" Login "></div></center>';
    print_footer();
}
